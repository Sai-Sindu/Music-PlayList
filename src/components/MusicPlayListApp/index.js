...
class MusicPlayListApp extends Component {
  state = {trackList: initialTracksList}

  onChangeSearchText = event => {
    this.setState(prevState => ({
      trackList: prevState.trackList.filter(eachMusic =>
  ...
  deleteMusic = id => {
    this.setState(prevState => ({
      trackList: prevState.trackList.filter(eachMusic => eachMusic.id !== id),
    }))
  }
  ...
  renderMusicListView = () => {
    ...
    return (
      <MusicPlayListCard>
        {trackList.length > 0 ? (
          trackList.map(eachMusic => (
            <MusicItem
              musicItemDetails={eachMusic}
              key={eachMusic.id}
